...
class hidalgo:
    ...
    def _fit(self, X):
        ...
        self.Nsamp = Nsamp
        Npar = N + 2 * K + 2 + 1 * (estimate_zeta) # questo non lo capisco proprio...
    ...
    def fit(self, X):
        ...
        K = self.K
        #################### prove /
        # print('length sampling: ',len(sampling[:, :K]))
        # print('sampling: ', sampling[:, :K])
        # self.Nsamp è il numero di sampling che tiene alla fine, cioè tolto 
        # il burn_in (qui espresso come percentuale sul totale)
        #################### prove /

        assign=sampling[:,2*K:2*K+N]
        ...
        pZ = np.max(Pi, axis=0)
        v_discrete=np.zeros(len(Z))
        v_discrete=np.zeros(len(Z))
        v_=np.zeros((self.Nsamp, len(Z)))
        for index_point in range (len(Z)): 
            v_discrete[index_point]=self.d_[Z[index_point]]
            v_discrete[index_point]=self.derr_[Z[index_point]]

        for index_point in range (len(Z)): 
            for index_samp in range (self.Nsamp):
                i_=int(assign[index_samp, index_point])
                v_[index_samp, index_point]=sampling[index_samp, i_]

        self.V=v_.mean(axis=0)
        self.Vstd=v_.std(axis=0)     
        self.Vmedian=np.median(v_, axis=0)         
        Z = Z + 1
        Z[np.where(pZ < 0.8)] = 0
        self.Z = Z
